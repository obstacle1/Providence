...
const fmtDate = (d) => {
  ...
  return p.length === 2 ? `${m} '${p[0].slice(2)}` : `${m} ${+p[2]}, ${p[0]}`;
};
...
// ── AI Fetch — two-step: search then extract ──────────────────────────────────
...
async function fetchComparables(artist, medium, category) {
  // Step 1: search for auction results
  ...
  // Step 2: convert prose results to clean JSON
  const jsonData = await callClaude([{
    role: "user",
...
Rules: trend must be "rising", "stable", or "declining". salePrice must be a USD number. Dates in YYYY-MM format. Return only the JSON object.`
  }], false);
...
// ── Design tokens ─────────────────────────────────────────────────────────────
const C = {
  bg: "#0F0E0C", card: "#161410", border: "#252019",
  gold: "#C9A84C", goldFaint: "#C9A84C22",
  text: "#E4DCCF", muted: "#8A7A68", dim: "#5A5044",
  green: "#6FA87A", red: "#A8706F",
  inner: "#121009", active: "#1C1914",
};

// ── Style factories ───────────────────────────────────────────────────────────
const mkBtn = (variant = "primary", extra = {}) => ({
  background: variant === "primary" ? C.gold : "transparent",
  color: variant === "primary" ? "#0F0E0C" : C.gold,
  border: `1px solid ${variant === "primary" ? C.gold : C.border}`,
  padding: "8px 16px", cursor: "pointer", fontSize: 11,
  letterSpacing: "0.1em", textTransform: "uppercase",
  borderRadius: 2, fontFamily: "Georgia, serif", whiteSpace: "nowrap",
  flexShrink: 0, ...extra,
});

const mkInput = (extra = {}) => ({
  background: C.inner, border: `1px solid ${C.border}`, color: C.text,
  padding: "8px 10px", borderRadius: 2, fontSize: 13, width: "100%",
...
const ChartTip = ({ active, payload, label: lbl }) => {
  ...
  return (
    <div style={{ background: "#1A1712", border: `1px solid ${C.goldFaint}`, padding: "9px 13px", borderRadius: 3, fontSize: 12 }}>
      <div style={{ color: C.gold, marginBottom: 3 }}>{lbl}</div>
...
function ComparablesPanel({ object }) {
  ...
  const run = async () => {
    setStatus("loading"); setErr(""); setPhase("Searching auction records…");
    try {
      // Small delay so user sees the phase message change
      const result = await fetchComparables(object.artist, object.medium, object.category);
  ...
  const currentVal = [...object.valuations]
    .sort((a, b) => a.date.localeCompare(b.date)).slice(-1)[0]?.value;
  ...
  const compChart = (data?.comparables || [])
    .filter((c) => c.salePrice && c.saleDate)
    .sort((a, b) => a.saleDate.localeCompare(b.saleDate))
    .map((c) => ({ date: fmtAxis(c.saleDate), comp: c.salePrice }));

  return (
    <div style={CARD}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 12 }}>
        <div style={{ fontSize: 9, letterSpacing: "0.22em", textTransform: "uppercase", color: C.dim }}>
          Market Comparables · AI
        </div>
        <button style={mkBtn(status === "done" ? "ghost" : "secondary", { fontSize: 10, padding: "5px 11px" })}
          onClick={run} disabled={status === "loading"}>
          {status === "loading" ? "Searching…" : status === "done" ? "↻ Refresh" : "Search Live Market"}
        </button>
      </div>

      {status === "idle" && (
        <div style={{ padding: "12px 0 2px", color: C.dim, fontSize: 12, lineHeight: 1.7 }}>
          Search recent auction sales for <span style={{ color: C.gold }}>{object.artist}</span> to see market comparables and price range.
        </div>
      )}

      {status === "loading" && (
        <div style={{ padding: "18px 0 4px", color: C.dim, fontSize: 12 }}>
          <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.2}}`}</style>
          <div style={{ color: C.gold, fontSize: 18, marginBottom: 8, animation: "pulse 1.4s infinite", textAlign: "center" }}>◎</div>
          <div style={{ textAlign: "center" }}>{phase}</div>
        </div>
      )}

      {status === "error" && (
        <div style={{ padding: "8px 0 2px", fontSize: 12, color: C.red }}>
          {err}.{" "}
          <span style={{ color: C.gold, cursor: "pointer", textDecoration: "underline" }} onClick={run}>Try again</span>
        </div>
      )}
        ...
        <>
          <div style={{ background: C.inner, border: `1px solid ${C.border}`, borderRadius: 2, padding: "11px 13px", marginBottom: 12 }}>
            <div style={{ display: "flex", gap: 18, flexWrap: "wrap", marginBottom: 8 }}>
              ...
              {data.lowEstimate && (
                <div><div style={LBL}>Comparable Range</div>
                  <div style={{ color: C.text, fontSize: 12 }}>{fmt(data.lowEstimate)} – {fmt(data.highEstimate)}</div></div>
              )}
              {vsMarket && (
                <div><div style={LBL}>Your Valuation</div>
                  <div style={{ color: vsMarket.color, fontSize: 13 }}>{vsMarket.label}</div></div>
              )}
            </div>
          ...
          <div style={{ fontSize: 10, color: C.dim, marginTop: 12, borderTop: `1px solid ${C.border}`, paddingTop: 9, lineHeight: 1.5 }}>
            Public auction records via live web search. For research only — not a formal appraisal.
          </div>
        </>
      )}
    </div>
...
export default function App() {
  ...
  const portfolioChart = useMemo(() => {
    ...
    return dates.map((date) => {
      ...
      objects.forEach((obj) => {
        ...
        const before = s.filter((v) => v.date <= date);
        if (before.length) total += before[before.length - 1].value;
      });
  ...
  const objectChart = useMemo(() => {
    if (!selected) return [];
    return [...selected.valuations]
      .sort((a, b) => a.date.localeCompare(b.date))
      .map((v) => ({ date: fmtAxis(v.date), value: v.value }));
  }, [selected]);

  const portStats = useMemo(() => {
    const totals = objects.map((o) => {
      const s = [...o.valuations].sort((a, b) => a.date.localeCompare(b.date));
      return { first: s[0]?.value || 0, last: s[s.length - 1]?.value || 0 };
    });
    const cur  = totals.reduce((acc, t) => acc + t.last, 0);
    const acq  = totals.reduce((acc, t) => acc + t.first, 0);
    const gain = cur - acq;
  ...
  const addValuation = () => {
    ...
    setObjects((p) => p.map((o) => o.id === selectedId
      ? { ...o, valuations: [...o.valuations, { ...newVal, value: +newVal.value }] } : o));
    setNewVal({ date: "", value: "", note: "" });
    setShowAddVal(false); notify("Valuation saved");
  };
  ...
  const importParse = () => {
    try {
      ...
      setImportMapped(rows); setImportStep(2); setImportError("");
  ...
  const importConfirm = () => {
    ...
    const grouped = {};
    importMapped.forEach((row) => {
      const key = `${row.title}|${row.artist}`;
      if (!grouped[key]) grouped[key] = { title: row.title, artist: row.artist, medium: row.medium || "", year: +(row.year || 0), category: row.category || "Painting", valuations: [] };
      if (row.date && row.value) grouped[key].valuations.push({ date: row.date, value: +row.value, note: row.note || "" });
    });
    const objs = Object.values(grouped).map((o, i) => ({ ...o, id: Date.now() + i }));
    setObjects((p) => [...p, ...objs]);
    setImportText(""); setImportMapped(null); setImportStep(1);
    setView("portfolio"); notify(`Imported ${objs.length} object(s)`);
  };
  ...
  return (
    // overflow:hidden on root stops horizontal bleed
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: "'Georgia', serif", overflowX: "hidden" }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.border}`, padding: "14px 16px", boxSizing: "border-box", width: "100%" }}>
        <div style={{ marginBottom: 11 }}>
          ...
          <div style={{ fontSize: 9, letterSpacing: "0.2em", color: C.dim, textTransform: "uppercase", marginTop: 2 }}>
            Collection Value Intelligence
          </div>
        </div>
        <div style={{ display: "flex", gap: 5, overflowX: "auto", paddingBottom: 1, WebkitOverflowScrolling: "touch" }}>
          {NAV.map(({ key, label, disabled }) => (
            <button key={key}
              style={{
                background: view === key ? C.gold : "transparent",
                color: view === key ? C.bg : disabled ? C.dim : C.muted,
                border: `1px solid ${view === key ? C.gold : C.border}`,
                padding: "6px 13px", cursor: disabled ? "default" : "pointer",
                fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase",
                borderRadius: 2, fontFamily: "Georgia, serif",
                whiteSpace: "nowrap", flexShrink: 0, opacity: disabled ? 0.4 : 1,
              }}
              onClick={() => { if (disabled) return; setView(key); }}
            >{label}</button>
          ))}
        </div>
      </div>

      {/* ── Page content — constrained, no overflow ── */}
      <div style={{ padding: "18px 16px", boxSizing: "border-box", width: "100%", maxWidth: 860, margin: "0 auto" }}>
        ...
        {view === "portfolio" && (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
              ...
              <StatCard lbl="Total Gain"       val={fmt(portStats.gain)}
                sub={`${portStats.gain >= 0 ? "▲" : "▼"} ${Math.abs(portStats.gainPct)}%`}
                subColor={portStats.gain >= 0 ? C.green : C.red} />
            ...
            {objects.map((obj) => {
              ...
              const fst = s[0]?.value || 0;
              const g   = fst ? pct(fst, cur) : null;
              return (
                <div key={obj.id}
                  style={{
                    display: "flex", alignItems: "center", justifyContent: "space-between",
                    padding: "12px", cursor: "pointer", borderRadius: 2, marginBottom: 4,
                    background: selectedId === obj.id ? C.active : "transparent",
                    border: `1px solid ${selectedId === obj.id ? C.gold + "44" : "transparent"}`,
          ...
          <>
            ...
            {objStats && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                ...
                <StatCard lbl="Total Change"  val={fmt(objStats.change)}
                  sub={`${objStats.change >= 0 ? "▲" : "▼"} ${Math.abs(objStats.changePct)}%`}
                  subColor={objStats.change >= 0 ? C.green : C.red} />
            ...
            <div style={CARD}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 12 }}>
                ...
                <button style={mkBtn("secondary", { fontSize: 10, padding: "5px 11px" })}
                  onClick={() => setShowAddVal((v) => !v)}>
                ...
                <div style={{ background: C.inner, border: `1px solid ${C.border}`, borderRadius: 2, padding: "13px", marginBottom: 13 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9, marginBottom: 9 }}>
                    <div>
                      ...
                      <input type="date" style={mkInput()} value={newVal.date} onChange={(e) => setNewVal({ ...newVal, date: e.target.value })} />
                    </div>
                    <div>
                      <label style={LBL}>Value (USD)</label>
                      <input type="number" style={mkInput()} placeholder="0" value={newVal.value} onChange={(e) => setNewVal({ ...newVal, value: e.target.value })} />
                    </div>
                    <div style={{ gridColumn: "1/-1" }}>
                      <label style={LBL}>Source / Note</label>
                      <input type="text" style={mkInput()} placeholder="e.g. Christie's appraisal" value={newVal.note} onChange={(e) => setNewVal({ ...newVal, note: e.target.value })} />
                    </div>
                  </div>
              ...
              {[...selected.valuations]
                ...
                .map((v, i, arr) => {
                  const prev = arr[i + 1];
                  const chg  = prev ? v.value - prev.value : null;
                  const chgP = prev ? pct(prev.value, v.value) : null;
                  return (
                    <div key={i} style={{
                      display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10,
                      padding: "10px 0", borderBottom: `1px solid ${C.border}`,
                      background: i % 2 ? C.inner + "88" : "transparent",
                    }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        {chg !== null ? (
                          <>
                            <div style={{ fontSize: 13, color: chg >= 0 ? C.green : C.red }}>{chg >= 0 ? "▲" : "▼"} {fmt(Math.abs(chg))}</div>
                            <div style={{ fontSize: 11, color: chg >= 0 ? C.green : C.red, marginTop: 1 }}>{Math.abs(chgP)}%</div>
                          </>
                        ) : (
                          <div style={{ fontSize: 11, color: C.dim }}>Acquisition</div>
                        )}
                      </div>
                    </div>
                  );
            ...
            <div style={CARD}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 11, marginBottom: 13 }}>
                <div style={{ gridColumn: "1/-1" }}>
                  ...
                  <input style={mkInput()} value={newObj.title} onChange={(e) => setNewObj({ ...newObj, title: e.target.value })} placeholder="Object title" />
                </div>
                <div>
                  <label style={LBL}>Artist / Maker</label>
                  <input style={mkInput()} value={newObj.artist} onChange={(e) => setNewObj({ ...newObj, artist: e.target.value })} placeholder="Name" />
                </div>
                <div>
                  <label style={LBL}>Year</label>
                  <input style={mkInput()} type="number" value={newObj.year} onChange={(e) => setNewObj({ ...newObj, year: e.target.value })} placeholder="e.g. 1952" />
                </div>
                <div>
                  <label style={LBL}>Medium</label>
                  <input style={mkInput()} value={newObj.medium} onChange={(e) => setNewObj({ ...newObj, medium: e.target.value })} placeholder="e.g. Oil on canvas" />
                </div>
                <div>
                  <label style={LBL}>Category</label>
                  <select style={mkInput()} value={newObj.category} onChange={(e) => setNewObj({ ...newObj, category: e.target.value })}>
        ...
        {view === "import" && (
          <div>
            ...
            <div style={{ fontSize: 12, color: C.dim, marginBottom: 18, lineHeight: 1.6 }}>
              Columns: <span style={{ color: C.gold }}>title, artist, medium, year, category, date, value, note</span>
            </div>
            {importStep === 1 && (
              <div style={CARD}>
                <label style={LBL}>Paste CSV</label>
                <textarea style={mkInput({ height: 160, resize: "vertical", display: "block", marginBottom: 11 })}
                  value={importText} onChange={(e) => setImportText(e.target.value)}
                  placeholder={"title,artist,medium,year,category,date,value,note\nBlue Study,Picasso,Oil,1903,Painting,2020-01-01,1200000,Christie's"} />
                {importError && <div style={{ color: C.red, fontSize: 12, marginBottom: 9 }}>{importError}</div>}
                <button style={mkBtn("primary")} onClick={importParse}>Parse</button>
              </div>
            )}
            {importStep === 2 && importMapped && (
              <div style={CARD}>
                <div style={SEC}>Preview — {importMapped.length} rows</div>
                <div style={{ overflowX: "auto", marginBottom: 14 }}>
                  <table style={{ borderCollapse: "collapse", fontSize: 11, minWidth: "100%" }}>
                    <thead><tr>{Object.keys(importMapped[0]).map((k) => (
                      <th key={k} style={{ padding: "4px 7px", textAlign: "left", color: C.dim, borderBottom: `1px solid ${C.border}`, whiteSpace: "nowrap" }}>{k}</th>
                    ))}</tr></thead>
                    <tbody>{importMapped.slice(0, 6).map((row, i) => (
                      <tr key={i}>{Object.values(row).map((v, j) => (
                        <td key={j} style={{ padding: "4px 7px", color: C.muted, borderBottom: `1px solid ${C.inner}`, whiteSpace: "nowrap" }}>{v}</td>
                      ))}</tr>
                    ))}</tbody>
                  </table>
                  {importMapped.length > 6 && <div style={{ color: C.dim, fontSize: 10, marginTop: 6 }}>…and {importMapped.length - 6} more</div>}
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={mkBtn("primary")} onClick={importConfirm}>Import All</button>
                  <button style={mkBtn("ghost")} onClick={() => setImportStep(1)}>Back</button>
                </div>
              </div>
            )}
          </div>
      ...
      {toast && (
        <div style={{
          ...
          boxShadow: "0 4px 14px #00000066", zIndex: 999,
        }}>{toast}</div>
      )}
    </div>
  );
