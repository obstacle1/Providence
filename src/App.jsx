    ...
    <div style={{ background:C.bg, minHeight:"100vh", color:C.text, fontFamily:"'Georgia', serif", overflowX:"hidden" }}>
      ...
      <div style={{ padding:"18px 16px", boxSizing:"border-box", width:"100%", maxWidth:860, margin:"0 auto" }}>
        ...
        {view==="portfolio"&&(<>
          ...
          {portfolioObjects.length>0&&<><div style={SEC}>Unassigned Objects</div>{portfolioObjects.map((obj)=>{ const s=[...obj.valuations].sort((a,b)=>a.date.localeCompare(b.date)), cur=s[s.length-1]?.value||0, fst=s[0]?.value||0, g=fst?pct(fst,cur):null; return (<div key={obj.id} style={{ borderRadius:2, marginBottom:4, background:C.card, border:`1px solid ${C.border}`, boxSizing:"border-box", overflow:"hidden" }}><div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px", cursor:"pointer" }} onClick={()=>{ setSelectedId(obj.id); setView("object"); }}><div style={{ flex:1, minWidth:0, paddingRight:10 }}><div style={{ fontSize:14, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{obj.title}</div><div style={{ fontSize:11, color:C.dim, marginTop:2 }}>{obj.artist} · {obj.year}</div></div><div style={{ textAlign:"right", flexShrink:0 }}><div style={{ fontSize:14, color:C.gold }}>{fmt(cur)}</div>{g!==null&&<div style={{ fontSize:11, color:cur>=fst?C.green:C.red, marginTop:1 }}>{cur>=fst?"▲":"▼"} {Math.abs(g)}%</div>}</div></div>{clients.length>0&&<div style={{ borderTop:`1px solid ${C.border}`, padding:"7px 12px", display:"flex", alignItems:"center", gap:8, background:C.inner }} onClick={e=>e.stopPropagation()}><span style={{ fontSize:10, color:C.dim, letterSpacing:"0.08em", textTransform:"uppercase", whiteSpace:"nowrap" }}>Assign to</span><select style={{ background:C.inner, border:`1px solid ${C.border}`, color:C.text, padding:"3px 6px", borderRadius:2, fontSize:11, flex:1, fontFamily:"Georgia, serif", outline:"none" }} defaultValue="" onChange={async(e)=>{ const cid=e.target.value; if(!cid) return; await db.patch("objects",obj.id,{ client_id:cid }); setObjects(p=>p.map(o=>o.id===obj.id?{...o,client_id:cid}:o)); notify("Assigned to client"); }}><option value="">— Select client —</option>{clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>}</div>); })}</>}
        </>)}
